...
public interface AppointmentRepository extends PagingAndSortingRepository<Appointment, UUID> {
	...
	@Query(value = "SELECT a FROM Appointment a WHERE a.employee.id = ?1 AND a.pharmacy.id=?2"
	...
	@Query(value = "SELECT a FROM Appointment a WHERE NOT (a.startDateTime >= ?2 OR a.endDateTime <= ?1) "
			+ " AND a.pharmacy.id = ?3 AND a.employee.id = ?4")
	List<Appointment> findAllAppointments(Date dateTimeFrom, Date dateTimeTo,UUID pharmacyId, UUID employeeId);
}
